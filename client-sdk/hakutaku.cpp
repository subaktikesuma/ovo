...
bool hak::driver::is_process_alive_pid(pid_t pid) {
	socklen_t alive = 0;
	if(getsockopt(sock, pid, (int) IS_PROCESS_ALIVE_PID, NULL, &alive) < 0) {
...
int hak::driver::attach_process_pid(pid_t pid) {
	socklen_t len = 0;
	char* data = new char[0];
	if(getsockopt(sock, pid, (int) ATTACH_PROCESS, data, &len) < 0) {
...
uintptr_t hak::driver::get_process_module_base(std::string_view module, int vm_flag) {
	uintptr_t addr = 0;
	...
	if(getsockopt(sock, vm_flag, (int) GET_PROCESS_MODULE_BASE, data, &len) < 0) {
...
// 读写10w次 70ms
size_t hak::driver::read_process_memory_ioremap(uintptr_t addr, void *buffer, size_t size) {
	if(getsockopt(sock, size, (int) READ_PROCESS_MEMORY_IOREMAP, (void*) addr, (socklen_t*) buffer) < 0) {
...
size_t hak::driver::write_process_memory_ioremap(uintptr_t addr, void *buffer, size_t size) {
	if(getsockopt(sock, size, (int) WRITE_PROCESS_MEMORY_IOREMAP, (void*) addr, (socklen_t*) buffer) < 0) {
...
int hak::driver::access_process_vm(pid_t from, uintptr_t from_addr, pid_t to, uintptr_t to_addr, size_t len) {
	...
	if(getsockopt(sock, len, (int) ACCESS_PROCESS_VM, &req, &data_len) < 0) {
...
size_t hak::driver::read_process_memory(uintptr_t addr, void *buffer, size_t size) {
	if(getsockopt(sock, size, (int) READ_PROCESS_MEMORY, (void*) addr, (socklen_t*) buffer) < 0) {
		std::cout << "read process memory failed: " << strerror(errno) << std::endl;
		return 0;
	}
	return size;
...
size_t hak::driver::write_process_memory(uintptr_t addr, void *buffer, size_t size) {
	if(getsockopt(sock, size, (int) WRITE_PROCESS_MEMORY, (void*) addr, (socklen_t*) buffer) < 0) {
		std::cout << "write process memory failed: " << strerror(errno) << std::endl;
		return 0;
	}
	return size;
...
int hak::driver::remap_memory(uintptr_t addr, size_t size, void **buffer) {
	if(getsockopt(sock, size, (int) REMAP_MEMORY, (void*) addr, (socklen_t*) addr) < 0) {
